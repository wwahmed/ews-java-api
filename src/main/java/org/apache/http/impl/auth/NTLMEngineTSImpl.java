 ...
 * 
 * In addition to the default {@link NTLMEngineImpl}, the {@link #getType1Message(String, String)} method creates a new instance and doesn't reuse an existing one.
 * Due to its final state we can not easily extend {@link NTLMEngineImpl} and overwrite the default behavior.
 * @since 4.1
 * @see <a href="https://github.com/OfficeDev/ews-java-api/pull/428">https://github.com/OfficeDev/ews-java-api/pull/428</a>
 */
@NotThreadSafe
final class NTLMEngineTSImpl implements NTLMEngine {
    ...
    static String getType1Message(final String host, final String domain) throws NTLMEngineException {
    	...
    	// GH-371: Fix the issue @codewheeney and @schakko reported.
    	// see also https://issues.apache.org/jira/browse/HTTPCLIENT-1686
        return new Type1Message().getResponse();
    }
    ...
    static class NTLMMessage {
        ...
        protected int readUShort(final int position) throws NTLMEngineException {
            return NTLMEngineTSImpl.readUShort(messageContents, position);
        }

        /** Read a ulong from a position within the message buffer */
        protected int readULong(final int position) throws NTLMEngineException {
            return NTLMEngineTSImpl.readULong(messageContents, position);
        }

        /** Read a security buffer from a position within the message buffer */
        protected byte[] readSecurityBuffer(final int position) throws NTLMEngineException {
            return NTLMEngineTSImpl.readSecurityBuffer(messageContents, position);
        }
